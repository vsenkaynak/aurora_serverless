...
Features:
- Strict input: only accepts {"cluster_id": "<DBClusterIdentifier>"}.
- Three windows (≈14d/30d/60d) with frozen timestamps per window and -5m CW skew.
- PI-aware metrics: DBLoad/DBLoadCPU (Q1/Q2) only if Performance Insights enabled.
- Serverless v2 uses actual ACU (Q3) when present; otherwise falls back to DBLoad/CPU.
- CPU-only fallback path builds ACU time series and costs when Q1/Q2/Q3 are missing.
- Spike-aware headroom (revised): lowers utilization target when p99 >> p95 (more buffer).
- Compares Standard vs I/O-Optimized storage modes (compute multipliers + I/O pricing rules).
- Provisioned selection: candidates cost-sorted, exclude *.medium, stop at first fit.
- Expanded families: r4, r5, r6g, r6i, r7g, x2g, z1d (no t2/t3).
- Costs reported as monthly_equivalent only; JSON output only.
"""
...
#   "both"     -> evaluate Standard + I/O-Optimized
...
# AAS→ACU sensitivity factors (used with DBLoad when estimating ACU on provisioned)
...
# I/O request price for Standard storage (no per-I/O charges for aurora-iopt1)
IO_PRICE_PER_MILLION = 0.20  # USD per 1,000,000 requests
CHARGE_IO_FOR_STANDARD = True

# ACU limits / granularity
DEFAULT_MIN_ACU = 0.5
DEFAULT_MAX_ACU = 64.0
ACU_STEP = 0.5

# CPU-aware ACU estimation (used when DBLoad absent)
USE_CPU_IN_ACU_ESTIMATE = True
CPU_TO_ACU_HEADROOM = 0.70  # scale CPU→ACU similar to AAS headroom

# Spike-aware headroom (REVISED: lower target when spiky -> more buffer)
SPIKE_AWARE_HEADROOM = True
HEADROOM_BASE = 0.70     # normal utilization target (≈70% of capacity)
HEADROOM_MIN  = 0.50     # do not go below 50% target
HEADROOM_MAX  = 0.80     # sanity cap if adjusted upwards elsewhere
SPIKE_RATIO_THRESHOLD      = 1.30   # treat as spiky when p99 >= 1.30 * p95
HEADROOM_DROP_AT_THRESHOLD = 0.05   # drop target by 5 points when just over threshold
...
# Starter hourly prices (USD/hour, us-east-1) — replace with your exact static prices later
INSTANCE_HOURLY_PRICES: Dict[str, float] = {
    # R4 (legacy; not supported with aurora-iopt1)
...
IO_OPTIMIZED_MULTIPLIERS_BY_CLASS: Dict[str, float] = {
    # Family-level multipliers (starter values)
...
AAS_HEADROOM = HEADROOM_BASE  # for legacy references; compute_effective_headroom() is used dynamically
...
def get_clients(region: str):
    ...
    return boto3.client("	rds".strip(), region_name=region), boto3.client("cloudwatch", region_name=region)
...
def fetch_metrics(cw, cluster_id: str, instance_ids: List[str], writer_instance_id: Optional[str],
    ...
    # Q3 always at cluster level (Serverless v2 ACU series, may be empty)
...
def estimate_acu_seconds(cluster_mode: str, cluster: dict, metrics: Dict[str, MetricSeries],
    ...
    # 1) Serverless v2 → actual ACU if present
    ...
    # 2a) Blend paths if both present (use max for safety)
    ...
    # 3) If no DBLoad and no writer CPU, fallback to max CPU across instances
...
def compute_effective_headroom(shape: WorkloadShape) -> float:
    ...
    log(f"[headroom] base={HEADROOM_BASE:.2f} → effective={h:.2f} "
...
def evaluate_one_window_with_storage(*, label: str, factor: float,
    ...
    # Refresh instance info to get writer id/class
    insts: List[dict] = []
    writer_id = None
    if cluster.get("DBClusterMembers"):
        for m in cluster["DBClusterMembers"]:
    ...
    # Preflight (optional diagnostics)
    _ = preflight_list_metrics(cw, db_identifier)

    # Q1/Q2 only if PI enabled on any instance
    ...
    # Determine sizing basis and dynamic headroom
    ...
    for ic, eff_price_per_hour in sorted_cands:
        ...
        if fits:
            ...
            log(f"[provisioned_select] basis={basis}, headroom={eff_headroom:.2f}, first_fit={ic}, hourly={eff_price_per_hour:.4f}, monthly_eq={monthly_eq:.4f}")
            break  # stop at first (cheapest-by-cost) fit

    # Current provisioned class (if any)
    current_prov = None
    if current_classes:
    ...
    wd = WindowDecision(
        ...
        provisioned_current=current_prov, headroom_effective=eff_headroom,
    ...
    log("[evaluate_one_window] compact:", {
        ...
        "basis": basis, "headroom": eff_headroom
    })
    return wd
    ...
    try:
        ...
        for storage_mode in modes_to_run:
            ...
            for label, days, period in WINDOWS:
                ...
                # Freeze window timestamps once per (storage_mode, window) with -5min skew
        ...
        # Post-processor: cheapest picks (by monthly_equivalent)
