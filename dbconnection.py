# lambda_function.py
"""
Aurora Serverless v2 vs Provisioned with Storage-Type comparison + Post-Processor (Python 3.12)
- Uses REAL ACU (ServerlessDatabaseCapacity) when cluster is Serverless v2.
- For Provisioned clusters, estimates ACU from DBLoad, snaps to valid ACU steps, caps by min/max.
- Compares:
    * Serverless v2 (ACU) vs Provisioned (instance)
    * Standard vs I/O-Optimized storage (aurora-iopt1)
- Includes I/O request costs for Standard storage (no I/O charges on aurora-iopt1)
- Multi-window (14d/30d/60d); AAS→ACU sensitivity; broad instance catalog
- STORAGE_COMPARISON_MODE lets you simulate Standard, I/O-Optimized, or BOTH.
- Post-processor picks cheapest overall + top-3.
- EXTRA LOGGING: print(asdict(...)) for cost objects and selections to simplify CloudWatch debugging.
"""
...
# Serverless v2 (Standard storage) price (compute)
SERVERLESS_ACU_PER_HOUR = 0.12  # USD per ACU-hour

# If I/O-Optimized, multiply ACU price by this factor
SERVERLESS_IOPT_MULTIPLIER = 1.17  # tweak to match your region
...
# ---- ACU snapping/limits ----
# Default bounds when estimating ACU for provisioned clusters
DEFAULT_MIN_ACU = 0.5
...
def estimate_acu_seconds(cluster_mode: str, cluster: dict, metrics: Dict[str, MetricSeries],
                         factor: float, min_acu_default: float, max_acu_default: float) -> Tuple[float, str]:
    if cluster_mode == "serverless-v2":
        ...
        print("[estimate_acu_seconds] Serverless mode but no ACU samples; falling back to DBLoad estimate")

    dbload = metrics.get("q1")
    if dbload and dbload.values:
        period = infer_period_from_series(dbload.timestamps)
        est = []
        ...
        print(f"[estimate_acu_seconds] Estimated from DBLoad; factor={factor:.2f}, step={ACU_STEP}, bounds=[{min_acu_default},{max_acu_default}], ACU-sec={acu_seconds:.0f}")
        return float(acu_seconds), f"Estimated ACU from DBLoad (factor={factor:.2f}, snapped {ACU_STEP} ACU)"
    print("[estimate_acu_seconds] No DBLoad/ACU data; ACU=0")
...
def calc_serverless_cost(acu_seconds: float, storage_type: str, io_requests: float) -> CostBreakdown:
    ...
    io_total = 0.0
    io_note = "no I/O charge (iopt1)" if storage_type == "aurora-iopt1" else "I/O charged"
    if storage_type != "aurora-iopt1" and CHARGE_IO_FOR_STANDARD:
        io_total = (io_requests / 1_000_000.0) * IO_PRICE_PER_MILLION
    total = compute_total + io_total
    cb = CostBreakdown(total, f"{acu_hours:.1f} ACU-hrs × ${acu_price:.4f}/ACU-hr + {io_note} ({io_requests/1_000_000:.3f}M req @ ${IO_PRICE_PER_MILLION:.2f}/M)")
...
def calc_provisioned_cost(compute_price_per_hour: float, hours: float, instance_count: int,
    ...
    if storage_type != "aurora-iopt1" and CHARGE_IO_FOR_STANDARD:
        io_total = (io_requests / 1_000_000.0) * IO_PRICE_PER_MILLION
    total = compute_total + io_total
    cb = CostBreakdown(total, f"{instance_count}× inst-hrs × ${compute_price_per_hour:.4f}/hr + {io_note} ({io_requests/1_000_000:.3f}M req @ ${IO_PRICE_PER_MILLION:.2f}/M)")
...
def evaluate_one_window_with_storage(*, label: str, factor: float,
    ...
    # Reuse described instances for consistency
    insts = []
    if cluster.get("DBClusterMembers"):
        for m in cluster["DBClusterMembers"]:
            iid = m["DBInstanceIdentifier"]
    ...
    instance_ids, writer_id, current_classes = [], None, []
    ...
    # ACU usage: REAL if serverless-v2; estimate if provisioned
    acu_seconds, acu_notes = estimate_acu_seconds(mode, cluster, metrics, factor, DEFAULT_MIN_ACU, DEFAULT_MAX_ACU)
...
def lambda_handler(event, context):
    try:
        ...
        print("[lambda_handler] Done (with post-processed recommendations)")
